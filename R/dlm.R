...
# Observed predictive density calculation
dlm_obs_pred_dens = function(dat, params){
    y = dat$y
    F.vec = dat$F
    G.mat = dat$G
...
# Get parameter values for smoothed DLM
...
# Forecast distribution, h steps after the end of the data
dlm_forecast = function(dat, params, h = 12){
    y = dat$y
    F.vec = dat$F
    G.mat = dat$G
    n = length(y)
